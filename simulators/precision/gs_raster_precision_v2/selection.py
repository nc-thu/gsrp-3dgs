...
def check_heldout_gate(
    ...
    """Deprecated compatibility gate for the pre-v1 held-out terminology.

    New experiments should call :func:`check_workload_gate`, which uses the
    workload-aware scene-equal and tail constraints from the public contract.
    """
    rows = list(rows)
    if not rows:
...
def check_workload_gate_legacy(
    rows: Iterable[dict[str, object]],
    mean_delta_psnr: float = -0.2,
    worst_delta_psnr: float = -0.5,
    mean_delta_ssim: float = -0.002,
) -> dict[str, object]:
    """Compatibility helper for callers that used the old gate thresholds."""
    constraints = WorkloadConstraints(
        camera_count=0,
        scene_count=0,
        worst_delta_psnr_db=worst_delta_psnr,
        scene_equal_delta_psnr_db=mean_delta_psnr,
        scene_equal_delta_ssim=mean_delta_ssim,
    )
    return check_workload_gate(rows, constraints)
