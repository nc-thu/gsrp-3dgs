...
def build_camera_ready_report(
    ...
    if len(final_rows) != 573:
        raise ValueError(f"expected the 573-camera workload, found {len(final_rows)} rows")
    ...
    decision = {
        ...
        "camera_count": len(final_rows),
        # Kept for callers that consumed the pre-v1 JSON schema.
    ...
    manifest = {
        ...
        "validation": "the complete 573-camera workload",
    ...
    manifest["private_build_provenance"] = "not distributed"
